...
grid_search_prep = GridSearchCV(final_pipeline, param_grid, cv=5, n_jobs=-1, verbose=2)
grid_search_prep.fit(Base.housing, Base.housing_labels)

some_data = Base.housing.iloc[:4]
some_labels = Base.housing_labels.iloc[:4]

print("Predictions:\t", grid_search_prep.best_estimator_.predict(some_data))
